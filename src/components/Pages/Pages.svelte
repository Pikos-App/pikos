<script lang="ts">
  ...
  $: currentFiles = $selectedFolder
    ? directoryFiles.filter((file) => !file.is_directory && file.path.startsWith($selectedFolder.path))
    : [];
</script>
...
<div class="h-full flex flex-col">
  ...
  <div class="flex-1 overflow-y-auto">
    ...
    {:else if !$selectedFolder}
      <div class="p-4 text-center text-gray-500">Select a folder to view files</div>
    ...
    {:else}
      {#each currentFiles as file (file.path)}
        <div class="border-b border-gray-200 last:border-b-0">
          <button
            class="w-full px-3 py-2 text-left hover:bg-gray-100 transition-colors {$selectedPage?.path === file.path
              ? 'bg-blue-100'
              : ''}"
