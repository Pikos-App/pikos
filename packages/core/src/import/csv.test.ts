...
describe("applyMappings", () => {
  ...
  function buildRepeatConfig(): CSVMappingConfig {
    return {
      columnMappings: [
        { csvHeader: "Title", pikosField: "title", sampleValues: [] },
        { csvHeader: "Start Date", pikosField: "scheduledStart", sampleValues: [] },
        { csvHeader: "Repeat", pikosField: "repeat", sampleValues: [] },
        { csvHeader: "Status", pikosField: "status", sampleValues: [] },
      ],
      detectedSource: "TickTick",
      valueMappings: [
        {
          entries: [
            { sourceValue: "0", targetValue: "not_started" },
            { sourceValue: "-1", targetValue: "done" },
          ],
          field: "status",
        },
      ],
  ...
  it("imports a TickTick weekly repeat with anchor date", () => {
    const rows = [
      {
    ...
    const plan = applyMappings(rows, buildRepeatConfig());
    expect(plan.pages[0]!.rrule).toBe("FREQ=WEEKLY;BYDAY=MO");
    expect(plan.warnings).toHaveLength(0);
    expect(plan.meta.transformations).toContain("1 recurring rules imported");
  });

  it("strips bare rrules without an RRULE: prefix", () => {
    const rows = [
      { Repeat: "FREQ=DAILY", "Start Date": "2026-06-01", Status: "0", Title: "Brush teeth" },
    ];
    const plan = applyMappings(rows, buildRepeatConfig());
    expect(plan.pages[0]!.rrule).toBe("FREQ=DAILY");
  });

  it("drops the rule and warns when there is no anchor date", () => {
    const rows = [{ Repeat: "RRULE:FREQ=WEEKLY", "Start Date": "", Status: "0", Title: "Floats" }];
    const plan = applyMappings(rows, buildRepeatConfig());
    expect(plan.pages[0]!.rrule).toBeNull();
    expect(plan.warnings.some((w) => w.message.includes("no start/due date"))).toBe(true);
  });

  it("drops the rule and warns when rrule is unparseable", () => {
    const rows = [
      { Repeat: "TOTALLY_NOT_VALID", "Start Date": "2026-06-01", Status: "0", Title: "Broken" },
    ];
    const plan = applyMappings(rows, buildRepeatConfig());
    expect(plan.pages[0]!.rrule).toBeNull();
    expect(plan.warnings.some((w) => w.message.includes("unsupported recurrence"))).toBe(true);
  });

  it("silently drops rrule on a completed historical occurrence (TickTick status -1)", () => {
    const rows = [
      {
        Repeat: "RRULE:FREQ=WEEKLY;BYDAY=MO",
    ...
    const plan = applyMappings(rows, buildRepeatConfig());
    expect(plan.pages[0]!.status).toBe("done");
    expect(plan.pages[0]!.rrule).toBeNull();
    expect(plan.warnings).toHaveLength(0);
  });
});
...
describe("applyMappings — date parsing", () => {
  ...
  it("parses ISO datetime with timezone offset", () => {
    ...
    // Converted to local time — format has no timezone offset
...
"","Work","Review PR","TEXT","","","N","","","","","0","-1","2026-02-01T09:00:00+0000","2026-02-15T11:00:00+0000","0","America/Los_Angeles","false","false",,,"list","103","101"`;
  ...
  it("maps all-day events as date-only (TickTick exports midnight UTC + Is All Day flag)", () => {
    const { plan } = runFullPipeline(ticktickCSV);
    // Second task: Is All Day = true, Due Date "2026-06-15T00:00:00+0000",
    // no Start Date. The Is All Day flag forces date-only parsing so the
    // calendar renders it in the all-day row rather than as a midnight
    // timed block (which would also shift dates in non-UTC zones).
    expect(plan.pages[1]!.scheduledStart).toBe("2026-06-15");
    expect(plan.pages[1]!.scheduledEnd).toBeNull();
  });

  it("preserves multi-day spans on all-day events without converting to timed", () => {
    const csv = `"Folder Name","List Name","Title","Start Date","Due Date","Is All Day","Priority","Status","Created Time"
"","Work","Conference","2026-03-15T00:00:00+0000","2026-03-17T00:00:00+0000","true","0","0","2026-01-10T08:00:00+0000"`;
    const { plan } = runFullPipeline(csv);
    expect(plan.pages[0]!.scheduledStart).toBe("2026-03-15");
    expect(plan.pages[0]!.scheduledEnd).toBe("2026-03-17");
  });
