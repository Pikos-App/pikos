...
/// Read the tail of the rotating log file so the frontend can copy it into
/// a bug report. Returns empty string if the file doesn't exist (fresh
/// install with no rotation yet). Truncates to the last 50 KB on a UTF-8
/// character boundary so the clipboard payload stays bounded — older
/// entries roll off as new ones are written, which matches what a user
/// describing a recent bug actually wants.
#[tauri::command]
pub async fn read_recent_logs(app: tauri::AppHandle) -> Result<String, String> {
    let log_path = app
        .path()
        .app_log_dir()
    ...
    let bytes = std::fs::read(&log_path).map_err(|e| format!("read log: {}", e.kind()))?;
    truncate_log_tail(&bytes, 50_000).map_err(|e| format!("utf8: {e}"))
}

/// Take the last `max` bytes of `bytes` as a String, walking forward to the
/// next valid UTF-8 boundary so a multi-byte char is never split. Pure helper
/// extracted from `read_recent_logs` so the boundary logic is unit-testable
/// without an `AppHandle`.
fn truncate_log_tail(bytes: &[u8], max: usize) -> Result<String, std::str::Utf8Error> {
    if bytes.len() <= max {
        return Ok(std::str::from_utf8(bytes)?.to_string());
    }
    let mut start = bytes.len() - max;
    // UTF-8 continuation bytes have the bit pattern 10xxxxxx. Step forward
    // past any so `start` lands on a leading byte. Worst case skips ≤3 bytes.
    while start < bytes.len() && (bytes[start] & 0b1100_0000) == 0b1000_0000 {
        start += 1;
    }
    let tail = std::str::from_utf8(&bytes[start..])?;
    Ok(format!("… (older entries truncated)\n{}", tail))
}

#[cfg(test)]
mod tests {
    use super::truncate_log_tail;

    #[test]
    fn returns_full_content_when_under_limit() {
        let result = truncate_log_tail(b"hello world", 100).unwrap();
        assert_eq!(result, "hello world");
    }

    #[test]
    fn truncates_when_over_limit_with_marker() {
        let bytes: Vec<u8> = (0..200).map(|i| b'a' + (i % 26) as u8).collect();
        let result = truncate_log_tail(&bytes, 50).unwrap();
        assert!(result.starts_with("… (older entries truncated)\n"));
        // Tail length: 50 bytes (no UTF-8 continuation, so no skip).
        assert_eq!(result.len(), "… (older entries truncated)\n".len() + 50);
    }

    #[test]
    fn never_splits_a_multibyte_char() {
        // "é" is 2 bytes (0xC3 0xA9). Build a buffer where the naive cut
        // (bytes.len() - max) would land in the middle of one.
        let mut bytes = vec![b'a'; 100];
        // Append 50 "é" so the trailing region is dense with multi-byte chars.
        for _ in 0..50 {
            bytes.extend_from_slice("é".as_bytes());
        }
        // Try a window that crosses several "é" boundaries.
        for max in 50..70 {
            let result = truncate_log_tail(&bytes, max).unwrap();
            // Must round-trip as valid UTF-8 — `from_utf8` already enforced
            // this, but assert decoded chars are all "é" or 'a' (no replacement).
            let tail = result.trim_start_matches("… (older entries truncated)\n");
            for c in tail.chars() {
                assert!(c == 'a' || c == 'é', "unexpected char {c:?} for max={max}");
            }
        }
    }
    ...
    fn handles_empty_buffer() {
        let result = truncate_log_tail(b"", 100).unwrap();
        assert_eq!(result, "");
    }
}
