// Heavy-load seed: exercises sidebar rendering, page list scroll, FTS5 under
// load, and calendar density.
...
function randScheduledStart(): string {
  ...
  const d = set(addDays(new Date(), offsetDays), {
    hours: faker.number.int({ max: 22, min: 6 }),
    milliseconds: 0,
    minutes: faker.number.int({ max: 3, min: 0 }) * 15,
    seconds: 0,
  });
...
function randScheduledEnd(start: string, durationMins: number): string {
  return addMinutes(new Date(start), durationMins).toISOString().replace("Z", "").slice(0, 19);
}

/** ISO timestamp N days ago at a random hour. */
function daysAgo(n: number): string {
  const d = set(addDays(new Date(), -n), {
    hours: faker.number.int({ max: 22, min: 7 }),
    milliseconds: 0,
    minutes: faker.number.int({ max: 59, min: 0 }),
    seconds: 0,
  });
  return d.toISOString().replace("Z", "").slice(0, 19);
}
...
function dateBetween(after: string, before: string): string {
  ...
  const spanMs = new Date(before).valueOf() - a.valueOf();
  return addMilliseconds(a, Math.random() * spanMs)
    .toISOString()
    .replace("Z", "")
    .slice(0, 19);
}
...
export async function seedStress(
  ...
  for (const pg of pages) {
    ...
    const completedAt = isDone(pg) ? dateBetween(pg.createdAt, updatedAt) : undefined;
