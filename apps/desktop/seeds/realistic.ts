...
export async function seedRealistic(adapter: StorageAdapter): Promise<void> {
  ...
  async function createWithSchedule(spec: PageSpec) {
    ...
    if (spec.start) {
      const isTimed = isTimedIso(spec.start);
  ...
  await createWithSchedule({
    content: doc(
      ...
      h3("Takeaways"),
      p(
        "Speed, offline, longevity, privacy, and user control are achievable today with SQLite + a local app shell. Multi-device sync and real-time collaboration are the hard ones \u2014 worth designing for later, not blocking on now."
      )
    ),
    folderId: reading.id,
    priority: 3,
