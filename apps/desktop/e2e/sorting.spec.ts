...
/** Open the sort dropdown and pick a mode by visible label. */
...
appTest("sort by Date orders the page list by schedule @tier2", async ({ app }) => {
  ...
  await expect(list.filter({ hasText: "alpha" })).toContainText(/Today|May/);
  await expect(list.filter({ hasText: "bravo" })).toContainText(/Tomorrow|May/);
