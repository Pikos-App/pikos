...
appTest(
  "right panel calendar mode persists across reload @tier2",
  async ({ app }) => {
    // Start in editor mode (default), flip to calendar via Cmd+Shift+C.
    ...
    // The shell remounts with calendar still selected — the user shouldn't
    // have to flip the panel back every launch.
    await expect(app.getByRole("region", { name: "Week calendar" })).toBeVisible();
    await expect(app.getByRole("button", { name: "Calendar view" })).toHaveAttribute(
      "aria-pressed",
      "true"
...
appTest("sidebar collapsed state persists across reload @tier2", async ({ app }) => {
  ...
  // Now the affordance is the inverse — "Expand sidebar" exists.
  await expect(app.getByRole("button", { name: "Expand sidebar" })).toBeVisible();

  await app.reload();
  await expect(app.getByRole("main", { name: "Workspace" })).toBeVisible();

  // After reload, sidebar is still collapsed — Expand button still present,
  // Inbox sidebar entry hidden until expanded.
  await expect(app.getByRole("button", { name: "Expand sidebar" })).toBeVisible();
...
appTest("calendar reference week persists across reload @tier2", async ({ app }) => {
  ...
  // The label has moved forward — capture so we can confirm reload restores it.
  ...
  // Same week heading after reload — the user lands back on the week they
  // were viewing, not today.
...
appTest("per-view sort mode persists across reload @tier2", async ({ app }) => {
  ...
  // Default sort label varies by build — capture, then change to "Title".
  await sortBtn.click();
  ...
  // After reload — same sort mode on Inbox.
  await expect(app.getByRole("button", { name: "Sort: title" })).toBeVisible();
});
