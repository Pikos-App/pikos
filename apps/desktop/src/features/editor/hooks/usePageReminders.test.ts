// usePageReminders — tests for per-page reminder business logic.
//
// Covers: add/remove, dedup, sentinel "None" state, reset-to-default,
// and derived state (isNone, hasCustomReminders, activeReminders).
...
describe("setNone", () => {
  ...
  it("replaces existing reminders with sentinel", async () => {
    ...
    // Storage should only have the sentinel
    const stored = await adapter.listPageReminders(PAGE_ID);
