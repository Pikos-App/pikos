...
// ---------------------------------------------------------------------------
// Helpers — minimal Tiptap editor for ProseMirror state construction
// ---------------------------------------------------------------------------
...
describe("getIndentLevel", () => {
  ...
  it("returns the indent value when set", () => {
    const editor = createTestEditor();
    // Manually set indent via transaction
    const { state } = editor;
...
// ---------------------------------------------------------------------------
// Tab key integration — full keydown simulation
// ---------------------------------------------------------------------------
...
describe("Tab key behavior", () => {
  ...
  it("Shift+Tab outdents block regardless of cursor position", () => {
    ...
    // Move cursor to middle then Shift+Tab
    editor.commands.setTextSelection(3);
...
describe("Backspace at position 0 with indent", () => {
  it("outdents instead of joining with previous block", () => {
    ...
    // Backspace at position 0 should outdent
    editor.commands.setTextSelection(1);
  ...
  it("allows normal backspace when indent is 0", () => {
    ...
    // Should join paragraphs (default behavior)
