...
export function SettingsPage() {
  ...
  // so the editor doesn't keep pointing at a now-deleted page.
  async function handleUndoImport() {
    const importedIds = lastImportResult?.pageIds ?? [];
    if (activePageId && importedIds.includes(activePageId)) setActivePage(null);
        ...
        <ImportPreviewModal
          ...
          onConfirm={(skipCompleted) => {
            const plan = skipCompleted
              ? {
                  ...importState.plan,
                  pages: importState.plan.pages.filter(isOpen),
                }
              : importState.plan;
            void executeImport(plan).then((ok) => {
              if (!ok) return;
              reset();
