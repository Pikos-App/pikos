...
import { useUndoDelete } from "./UndoDeleteContext";
...
async function setup() {
  ...
  let folderId!: string;
  await act(async () => {
    const folder = await hook.result.current.pages.createFolder({ name: "Work" });
    folderId = folder.id;
    const page = await hook.result.current.pages.createPage({
      folderId: folder.id,
      title: "Test Page",
    });
    pageId = page.id;
...
describe("requestDeletePage", () => {
  ...
  it("leaves the active page alone when a different page is deleted", async () => {
    ...
    let otherId!: string;
    await act(async () => {
      const other = await hook.result.current.pages.createPage({ title: "Other" });
      otherId = other.id;
    });
...
describe("requestDeleteFolder", () => {
  it("hides the folder and creates a toast item with page count suffix", async () => {
    ...
    const folder = hook.result.current.pages.folders.find((f) => f.id === folderId)!;
  ...
  it("leaves the active page alone when it lives in a different folder", async () => {
    ...
    let otherId!: string;
    await act(async () => {
      const otherFolder = await hook.result.current.pages.createFolder({ name: "Other" });
      const other = await hook.result.current.pages.createPage({
