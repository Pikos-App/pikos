...
describe("blocking dialog open", () => {
  function openDialog(slot = "dialog-content"): HTMLElement {
  ...
  it("suppresses a global shortcut while a Dialog is open", () => {
    const handler = vi.fn();
  ...
  it("still fires shortcuts that opted in with allowInInputs (the dialog's own keys)", () => {
    const handler = vi.fn();
    Keyboard.register({ allowInInputs: true, combo: "Enter", handler, id: "test-dialog-confirm" });
    openDialog();

    Keyboard.handle(makeKeyEvent("Enter"));
    expect(handler).toHaveBeenCalledOnce();
  });

  it("does not suppress while the content is mid-close (data-state=closed)", () => {
    const handler = vi.fn();
    Keyboard.register({ combo: "Mod+Backspace", handler, id: "test-delete" });
    const el = openDialog();
  ...
  it("fires normally once the dialog is gone", () => {
    const handler = vi.fn();
    Keyboard.register({ combo: "Mod+Backspace", handler, id: "test-delete" });
  ...
  it("ignores non-dialog popovers (Radix Popover, not a blocking modal)", () => {
    const handler = vi.fn();
    Keyboard.register({ combo: "Mod+Backspace", handler, id: "test-delete" });
...
describe("Space key", () => {
  it("matches a 'Space' combo against the literal ' ' KeyboardEvent.key", () => {
    const handler = vi.fn();
    Keyboard.register({ combo: "Space", handler, id: "test-space" });
    Keyboard.handle(makeKeyEvent(" "));
    expect(handler).toHaveBeenCalledOnce();
  });

  it("supports modifier combos like 'Mod+Space'", () => {
    const handler = vi.fn();
    Keyboard.register({ combo: "Mod+Space", handler, id: "test-mod-space" });
