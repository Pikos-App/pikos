...
//
// Dev: routes to console.* so DevTools renders clickable stack traces.
// Prod: routes to tauri-plugin-log → ~/Library/Logs/com.pikos.app/pikos.log
//   (rotating, 2 MB max, KeepOne strategy — see lib.rs).
...
function emit(level: Level, scope: string, msg: string, err?: unknown): void {
  ...
  // Prod: dynamic import keeps tauri-plugin-log out of dev/test bundles
  // and avoids a top-level await on a Tauri command in non-Tauri contexts.
  void import("@tauri-apps/plugin-log")
    ...
    .catch(() => {
      // Plugin unavailable (non-Tauri runtime). Swallow — nothing to do.
    });
}
