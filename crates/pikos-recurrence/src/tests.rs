//! Engine tests — ported from `packages/core/src/utils/recurrence.test.ts` so
//! the native and wasm builds are pinned to the exact same behavior the JS
//! suite verifies.
...
fn next_monday_after_given_date() {
    ...
    assert_eq!(next.as_deref(), Some("2026-03-09T09:00:00"));
}
...
fn skips_missed_occurrences() {
    ...
    assert_eq!(next.as_deref(), Some("2026-03-23T09:00:00"));
}

#[test]
fn next_handles_all_day() {
    let next = next_occurrence_after("FREQ=WEEKLY;BYDAY=FR", "2026-03-06", "2026-03-06T00:00:00", &[]);
    assert_eq!(next.as_deref(), Some("2026-03-13"));
}

#[test]
fn next_handles_daily() {
    let next = next_occurrence_after("FREQ=DAILY", "2026-03-02T08:00:00", "2026-03-05T00:00:00", &[]);
    assert_eq!(next.as_deref(), Some("2026-03-06T08:00:00"));
}
...
fn final_occurrence_within_count() {
    ...
    assert_eq!(next.as_deref(), Some("2026-03-04T09:00:00"));
}
...
fn skips_consecutive_exdates() {
    ...
    assert_eq!(next.as_deref(), Some("2026-03-06T09:00:00"));
}
...
#[test]
fn snaps_weekend_anchor_for_weekday_rule() {
    assert_eq!(
        snap_anchor_to_rule("FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR", "2026-06-07"),
        "2026-06-08"
    );
}
...
// ─── rrule_to_label ─────────────────────────────────────────────────────────
// Golden strings generated from rrule.js `toText()` — keep byte-identical.

#[test]
fn labels_match_rrule_js() {
    let cases = [
        ...
        ("FREQ=WEEKLY;BYDAY=MO;COUNT=1", "every week on Monday for 1 time"),
        ("FREQ=DAILY;COUNT=10", "every day for 10 times"),
        ("FREQ=WEEKLY;UNTIL=20260628T235959Z", "every week until June 28, 2026"),
        ("FREQ=MONTHLY;INTERVAL=6;COUNT=2", "every 6 months for 2 times"),
        ("FREQ=WEEKLY", "every week"),
        ("FREQ=WEEKLY;INTERVAL=2;BYDAY=MO,TU,WE,TH,FR", "every 2 weeks on weekdays"),
        ("FREQ=YEARLY;INTERVAL=3;UNTIL=20300101T000000Z", "every 3 years until January 1, 2030"),
        ("FREQ=WEEKLY;BYDAY=SA,SU", "every week on Saturday, Sunday"),
    ];
    for (rrule, expected) in cases {
        assert_eq!(rrule_to_label(rrule).as_deref(), Some(expected), "label for {rrule}");
    }
    assert_eq!(rrule_to_label("INVALID_RRULE"), None);
}

// ─── parse_options / build_rrule ────────────────────────────────────────────
...
fn parses_freq_and_defaults_interval() {
    for (s, freq) in [
        ...
        let opts = parse_options(s).unwrap();
        assert_eq!(opts.freq, freq);
...
fn parses_interval_byday_count_until() {
    assert_eq!(parse_options("FREQ=WEEKLY;INTERVAL=3").unwrap().interval, 3);
    assert_eq!(
        parse_options("FREQ=WEEKLY;BYDAY=MO,WE,FR").unwrap().byweekday,
        Some(vec![0, 2, 4])
    );
    let counted = parse_options("FREQ=DAILY;COUNT=10").unwrap();
    assert_eq!(counted.count, Some(10));
    assert_eq!(counted.until, None);
    let until = parse_options("FREQ=WEEKLY;UNTIL=20260615T235959Z").unwrap();
...
fn parse_rejects_invalid_and_freqless() {
    assert_eq!(parse_options("NOT_A_RULE"), None);
    assert_eq!(parse_options("INTERVAL=2"), None);
}

#[test]
fn parse_ignores_byday_ordinals_and_unknown_keys() {
    assert_eq!(
        parse_options("FREQ=MONTHLY;BYDAY=2MO;BYMONTHDAY=15;WKST=SU")
            .unwrap()
            .byweekday,
        Some(vec![0])
    );
...
fn roundtrips_parse_build() {
    for original in [
        ...
        let parsed = parse_options(original).unwrap();
        let rebuilt = build_rrule(&parsed);
        assert_eq!(parse_options(&rebuilt).unwrap(), parsed, "roundtrip for {original}");
    }
}
