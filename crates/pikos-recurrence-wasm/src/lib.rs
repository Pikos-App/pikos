...
pub fn expand_range(
    ...
    let occurrences = pikos_recurrence::expand_range(
        rrule,
        scheduled_start,
    ...
    serde_json::to_string(&occurrences).unwrap_or_else(|_| "[]".to_string())
}
...
pub fn missed_occurrences_between(
    ...
    let missed = pikos_recurrence::missed_occurrences_between(
        rrule,
        scheduled_start,
        after,
        before,
...
/// Typed options as a JSON object (`{freq, interval, byweekday?, count?,
/// until?}`), or undefined when unparseable / unsupported FREQ.
#[wasm_bindgen(js_name = parseRruleOptions)]
pub fn parse_rrule_options(rrule: &str) -> Option<String> {
    pikos_recurrence::parse_options(rrule)
...
/// can't be reduced to the supported subset (callers fall back to the raw
/// string).
