...
- **Completion model**: Non-recurring pages: `pages.status`, completable from page list checkbox or calendar block hover. Recurring pages: "head + clone-on-complete" — completing the head clones it as a done page and advances `scheduledStart` to the next future occurrence. Head ID is stable. Virtual occurrences are not independently completable. See `features/recurring-completion.md`.
...
- **Quick Add confirmation**: Removed. All types (single, finite, recurring) create immediately on Enter. The byline chips show recurrence label before submission so users can verify.
...
- **Large document performance ceiling**: ProseMirror/Tiptap renders the entire document in the DOM — no viewport virtualization. Performance degrades around 300k words (slow open, slow editing, slow search, slow teardown when switching away). Obsidian avoids this via CodeMirror 6 which virtualizes the DOM (only renders visible lines). Switching to CM6 would solve the problem but loses WYSIWYG rich text — wrong trade-off for Pikos's audience. Accepted limitation for V1. Mitigation: encourage document splitting via linked sub-pages. If it becomes a real user complaint, revisit with options: soft warning at ~50k words, lazy JSON parsing via web worker, or document chunking.
